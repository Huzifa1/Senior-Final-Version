<?php
	...
	if(mysqli_num_rows($result)!==0){
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			$string1 = "window.location.replace('update.php?id=".$row["visit_id"]."&status=accepted&email=".$row["email"]."&name=".$row2['priosner_name']."&date=".$row['visit_date']."')";
			$string2 = "window.location.replace('update.php?id=".$row["visit_id"]."&status=rejected&email=".$row["email"]."&name=".$row2['priosner_name']."&date=".$row['visit_date']."')";
